    ...
    public class GradientPathBrush : FrameworkElement
    {
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            foreach (PathFigure pathFigure in pathGeometry.Figures)
            {
                ...
                for (int k = 0; k < pathFigure.Segments.Count; k++)
                {
                    if (k % 2 == 0)
                    {
                        PolyLineSegment? polylineSegment = pathFigure.Segments[k] as PolyLineSegment;

                        if (polylineSegment == null)
                            throw new NotSupportedException("PolylineSegment is null");
                        PointCollection segPoints = polylineSegment.Points;
                        foreach (var point in segPoints)
                        {
                            points.Add(point);
                        }
                    }
                ...
                //PolyLineSegment polylineSegment = pathFigure.Segments[xx] as PolyLineSegment;
